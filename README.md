...
- `wordSets`：单词集（课程）
- `words`：单词数据
- `wordProgress`：单词学习进度（核心调度表）
- `reviewPlans`：复习计划（艾宾浩斯遗忘曲线）
...
2. **性能监控**：关键操作性能监控和统计
3. **数据库索引**：优化查询性能，减少全表扫描
4. **懒加载**：组件和数据的按需加载
5. **React 优化**：使用 `React.memo`、`useMemo`、`useCallback` 优化渲染

### 性能指标

- **数据库查询**：索引优化后，查询响应时间 < 50ms
