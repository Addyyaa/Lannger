...
- **智能复习通知**：到达复习时间点后自动提醒，支持多课程复习队列管理
  - **页面内通知**：在应用内显示复习提醒卡片
  - **系统通知栏**：在手机通知栏显示复习提醒（需授权），即使应用关闭也能收到
...
- **弹窗组件**：使用 React Portal 实现居中显示，避免样式冲突
- **无障碍支持**：遵循 WCAG 标准，确保良好的可访问性

### 📦 数据管理

- **批量导入**：支持 Excel、CSV、JSON、XML 等多种格式导入词库
- **智能单词集匹配**：导入时支持通过 ID 或名称匹配已有单词集，自动处理冲突（重命名/覆盖/跳过）
- **字段验证**：例句（example）字段为可选，其他必填字段（kana、kanji、meaning）自动验证
- **单词集管理**：按教材单元、主题等方式组织学习内容
- **数据导出/导入**：支持完整数据备份和恢复
- **数据完整性检查**：自动检测和修复数据一致性问题
- **数据归档**：自动归档 90-365 天的复习日志，优化存储性能
...
- **系统通知**：支持在手机通知栏显示复习提醒，即使应用关闭也能收到通知
  - 自动请求通知权限（首次使用时）
  - 智能防重复：同一单词集 5 分钟内只通知一次
  - 通知操作：点击通知可直接开始复习或稍后提醒
  - 支持震动和声音提醒（移动设备）
...
- **数据库**：IndexedDB（通过 Dexie ORM，当前版本 v7）
- **路由**：React Router v7
...
- `reviewLogs`：复习日志（详细记录，支持归档）
- `reviewLogsArchive`：复习日志归档表（v7 新增，归档 90-365 天的日志）
- `dailyStats`：每日统计
...
- **通知服务** (`notificationService.ts`)
  ...
  - 通知权限管理
  - 通知点击事件处理
  - 防重复通知机制（5 分钟内同一单词集只通知一次）

- **数据归档服务** (`archiveService.ts`)

  - 自动归档历史复习日志（90-365 天）
  - 优化数据库存储性能

- **通告服务** (`announcementService.ts`)
...
- **数据迁移**：支持数据库版本升级和数据迁移（当前版本 v7）
- **数据完整性**：自动检测和修复数据一致性问题（`dataIntegrity.ts`）
- **级联删除**：删除单词/单词集时自动清理相关数据
- **数据归档**：自动归档历史复习日志，优化存储性能（`archiveService.ts`）
- **数据备份**：支持完整数据导出和导入（JSON 格式）
...
- ✅ **导入功能增强**：支持 wordSet 字段作为 ID 或名称智能匹配已有单词集
- ✅ **数据归档**：复习日志自动归档功能（v7）
- ✅ **UI 优化**：弹窗组件使用 Portal 实现居中显示，例句字段改为可选
...
**最后更新**：2025-11-26  
**当前版本**：0.0.33（查看 `version.json` 获取最新版本信息）  
**数据库版本**：v7  
