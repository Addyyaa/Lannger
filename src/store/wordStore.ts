...
import { db, WordSet, Word } from "../db";
 ...
 * @returns 
 */
export async function createWordSet(wordSet: Omit<WordSet, "id" | "createdAt" | "updatedAt">) {
    const newWordSet = {
        ...wordSet,
 ...
 * @returns 
 */
export async function createWord(word: Omit<Word, "id" | "createdAt" | "updatedAt">) {
    const newWord = {
        ...word,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
...
export async function fuzzySearchWordSets(query: string) {
    return await db.wordSets.filter((wordSet) => wordSet.name.includes(query)).toArray();
}
export async function fuzzySearchWords(query: string) {
    return await db.words.filter((word) => word.kana.includes(query) || word.kanji?.includes(query) || word.meaning.includes(query)).toArray();
}
...
export async function getWordSetByCommon(keyword: string) {
    ...
    const wordsByKanji = await db.words.where("kanji").startsWith(keyword).toArray();
    if (wordsByKanji.length > 0) { return wordsByKanji; }
    const wordsBySetName = await db.wordSets.where("name").startsWith(keyword).toArray();
    if (wordsBySetName.length > 0) { return wordsBySetName; }
    const wordsByMeaning = await db.words.where("meaning").startsWith(keyword).toArray();
    if (wordsByMeaning.length > 0) { return wordsByMeaning; }
    return [];
}
...
export async function getWordByIndex(setId: number, kanaPrefix: string) {
    return await db.words.where("[setId+kana]").between([setId, kanaPrefix], [setId, kanaPrefix + "\uffff"]).toArray();
}
...
export async function getWordSetsByTime(startTime: string, endTime: string) {
    ...
    return await db.wordSets.where("createdAt").between(startTime, endTime).toArray();
}
export async function getWordsByTime(startTime: string, endTime: string) {
    if (!dataVerify(startTime) || !dataVerify(endTime)) {
        throw new Error("startTime and endTime must be valid dates");
    }
    return await db.words.where("createdAt").between(startTime, endTime).toArray();
}
