...
- **核心逻辑**：当用户完成所有需要复习的单词后，系统需要根据复习单词的掌握程度决定是否退出复习流程
- **完成条件**：只有复习的单词**全部掌握**（所有单词的结果都是 `correct`）时，才可以退出本次复习并标记当前艾宾浩斯遗忘曲线的阶段完成
- **未完成处理**：如果存在未掌握的单词（结果为 `wrong` 或 `skip`），需要继续复习这些单词，直到全部掌握
- **用户体验**：
  - 当所有单词复习完成但未全部掌握时，显示提示信息："还有 X 个单词未掌握，请继续复习"
  - 自动将未掌握的单词重新加入复习队列
  - 显示未掌握单词的列表，方便用户查看
  - 只有全部掌握后，才显示"复习完成"提示并退出

**业务价值**：

- **确保复习质量**：通过强制要求全部掌握，确保用户真正理解每个单词，而不是仅仅完成数量
- **提升学习效果**：符合艾宾浩斯遗忘曲线的科学原理，只有掌握后才能进入下一阶段
- **降低遗忘率**：避免用户带着未掌握的单词进入下一阶段，导致后续复习困难

**用户故事**：

> 作为一名学习者，当我完成一轮复习后，我希望系统能够检查我的掌握情况。如果还有单词未掌握，我希望系统能够提醒我继续复习，直到我真正掌握所有单词。只有这样，我才能进入下一阶段的复习，确保学习效果。

**技术实现要点**：

1. **复习结果跟踪**：
   ```typescript
   // 在 ReviewStudy 组件中维护复习结果映射
...
2. **完成条件判断**：
   ...
   /**
    * 检查是否所有单词都已掌握
    * @param wordIds 本次复习的单词ID列表
    * @param reviewResults 复习结果映射
    * @returns { allMastered: boolean, unmasteredWordIds: number[] }
    */
   function checkMasteryStatus(
     wordIds: number[],
     reviewResults: Map<number, ReviewResult>
   ): { allMastered: boolean; unmasteredWordIds: number[] } {
     const unmasteredWordIds: number[] = [];
     
     for (const wordId of wordIds) {
       const result = reviewResults.get(wordId);
...
3. **修改复习完成处理逻辑**：
   ...
   /**
    * 处理复习完成
    * 只有所有单词都掌握时，才标记阶段完成并退出
    */
   const handleReviewComplete = async () => {
     try {
       // 检查掌握情况
       const { allMastered, unmasteredWordIds } = checkMasteryStatus(
         wordIds,
         reviewResults
       );
       ...
       if (!allMastered) {
         ...
         // 显示提示信息
         showNotification(
           `还有 ${unmasteredWordIds.length} 个单词未掌握，请继续复习`
         );
         return;
       }
       ...
       // 显示完成提示
       showSuccessMessage("恭喜！本次复习全部掌握，已进入下一阶段");
...
5. **UI 提示设计**：
   - 当检测到未掌握单词时，显示模态提示框：
     - 标题："还有单词未掌握"
     - 内容：显示未掌握单词的数量和列表（可选）
     - 按钮："继续复习"（自动将未掌握单词加入队列）
   - 当全部掌握时，显示成功提示：
     - 标题："复习完成！"
     - 内容："恭喜！所有单词都已掌握，已进入下一阶段"
     - 按钮："确定"（退出复习）
...
1. **用户跳过单词**：
   - `skip` 视为未掌握，需要重新复习
   - 如果用户多次跳过同一单词，记录跳过次数，在提示中显示

2. **用户中途退出**：
   - 如果用户关闭复习界面，已复习的结果应保存
   - 下次继续复习时，只复习未掌握的单词

3. **单词数量为 0**：
   - 如果没有需要复习的单词，直接标记阶段完成

4. **重复复习**：
   - 如果用户对同一单词多次复习，以最后一次结果为准

**验收标准**：

- [ ] 当所有单词复习完成但未全部掌握时，系统能够正确识别未掌握的单词
- [ ] 未掌握的单词能够自动重新加入复习队列
- [ ] 用户能够看到清晰的提示信息，了解未掌握单词的数量
- [ ] 只有全部掌握后，才能标记阶段完成并退出复习
- [ ] 阶段完成标记正确更新到 `ReviewPlan` 表
- [ ] 复习锁定状态在阶段完成后正确解除
- [ ] 用户体验流畅，提示信息友好清晰
...
- 依赖任务 2.4.1（创建复习学习组件）
- 依赖任务 2.1.1（实现艾宾浩斯遗忘曲线算法）
- 依赖任务 2.2.1（实现复习锁定机制）

**风险与缓解**：

- **风险**：用户可能因为无法全部掌握而无法退出复习，导致挫败感
  - **缓解**：提供清晰的进度提示，显示已掌握/未掌握的比例，鼓励用户继续学习
  - **缓解**：允许用户查看未掌握单词的列表，帮助用户有针对性地复习

- **风险**：如果单词数量很多，用户可能需要多次循环复习
  - **缓解**：优化复习算法，优先显示未掌握的单词
  - **缓解**：提供"查看未掌握单词"功能，让用户快速定位

**后续优化建议**：

- 考虑引入"掌握阈值"概念：如果掌握率达到 90% 以上，是否允许进入下一阶段（需要产品决策）
- 记录用户完成一轮复习所需的时间，用于分析学习效率
- 提供"复习报告"功能，显示本次复习的详细统计信息
